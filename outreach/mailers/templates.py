...
REASON_MAP: dict[str, str] = {
    "no_ssl":    "SSLが未対応（HTTP）のため、ブラウザに「保護されていない通信」と警告が表示されます",
    "old_tech":  "古い技術（フレームセット・Flash・非推奨HTML要素など）が使われており、現代のブラウザで正常に表示されない可能性があります",
    "outdated":  "サイトの著作権表示が古く、長期間更新されていない可能性があります",
    "old":       "サイトの情報が古い、もしくは古い技術が使われている可能性",
    "no_mobile": "スマートフォン非対応（閲覧時にレイアウトが崩れる可能性）",
    "slow":      "サイトの表示速度が遅く、ユーザーが離脱している可能性があります",
    "no_line":   "LINE公式アカウントが未連携のため、LINE経由の集客・予約受付ができていません",
}
...
def select_template_key(site_status: str, industry: str) -> str:
    ...
    選択ルール:
      - none                  → A（サイトなし）
      - no_ssl / old_tech / outdated / old / no_mobile → B（サイト改善提案）
      - phone_only + 医療系   → D（LINE予約訴求）
      - phone_only + 一般     → C（24時間WEB予約訴求）
      - slow / no_line        → B（改善余地あり）
      - ok / error / その他   → B（改善余地あり）

    Args:
        site_status: "none"/"no_ssl"/"old_tech"/"outdated"/"old"/"no_mobile"/
                     "phone_only"/"slow"/"no_line"/"ok"
        industry:    業種名
    ...
    if site_status == "none":
        return "A"
    if site_status in ("no_ssl", "old_tech", "outdated", "old", "no_mobile"):
        return "B"
    if site_status == "phone_only":
        return "D" if is_medical(industry) else "C"
    # slow / no_line / ok / error / その他 → 改善余地ありとしてB
    return "B"
