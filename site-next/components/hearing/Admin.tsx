...
function statusStyle(s: string) {
  const found = STATUSES.find(x => x.label === s)
  return found ?? { color: '#9ca3af', bg: '#f9fafb' }
}
...
function Tag({ label, color = '#1a2540', bg = '#e8f0fe' }: { label: string; color?: string; bg?: string }) {
  return (
    <span style={{
...
export default function HearingAdmin() {
  ...
  const memoTimer = useRef<NodeJS.Timeout | null>(null)

  // 一覧取得
  useEffect(() => {
    fetch('/api/hearing')
      .then(r => r.json())
      .then((data: Hearing[]) => { setList(data); setLoading(false) })
  ...
  const updateStatus = async (id: number, status: string) => {
    await fetch(`/api/hearing/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status }),
    })
    const update = (h: Hearing) => h.id === id ? { ...h, status } : h
  ...
  const saveMemo = async () => {
    ...
    await fetch(`/api/hearing/${selected.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ memo }),
    })
    setList(l => l.map(h => h.id === selected.id ? { ...h, memo } : h))
  ...
  const row: React.CSSProperties = {
    ...
    borderBottom: '1px solid #f1f5f9', padding: '0.85rem 1rem',
    cursor: 'pointer',
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button
            onClick={() => {
              setLoading(true)
              fetch('/api/hearing').then(r => r.json()).then((data: Hearing[]) => { setList(data); setLoading(false) })
            }}
            style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading && (
            <p style={{ padding: '1.5rem', color: '#94a3b8', fontSize: '0.82rem', textAlign: 'center' }}>読み込み中...</p>
          )}
          {!loading && list.length === 0 && (
            <p style={{ padding: '1.5rem', color: '#94a3b8', fontSize: '0.82rem', textAlign: 'center' }}>回答はまだありません</p>
          )}
          {list.map(h => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderLeft: isSelected ? '3px solid #1a2540' : '3px solid transparent',
                }}
                onClick={() => select(h)}
              >
      ...
      {/* ─── 右：詳細 ──────────────────────────────────────────────────────── */}
      <div style={{ ...col, background: '#f8fafc', overflowY: 'auto', padding: '1.5rem' }}>
        {!selected ? (
          ...
          <div style={{ maxWidth: 720 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1rem', border: '1px solid #e2e8f0' }}>
              <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', color: '#94a3b8', marginBottom: '0.65rem' }}>STATUS</div>
            ...
            {/* 基本情報 */}
            <div style={{ background: '#fff', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1rem', border: '1px solid #e2e8f0' }}>
              <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', color: '#94a3b8', marginBottom: '0.75rem' }}>基本情報</div>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                <tbody>
                  {[
                    ['担当者', selected.name],
                    ['業種', selected.industry_other ? `${selected.industry}（${selected.industry_other}）` : selected.industry],
                    ['HP URL', selected.url ? <a href={selected.url} target="_blank" rel="noreferrer" style={{ color: '#3b82f6' }}>{selected.url}</a> : '—'],
                    ['流入元', selected.source ?? '—'],
                    ...
                    <tr key={String(lbl)} style={{ borderBottom: '1px solid #f1f5f9' }}>
            ...
            {selected.issues?.length > 0 && (
              <div style={{ background: '#fff', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1rem', border: '1px solid #e2e8f0' }}>
                <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', color: '#94a3b8', marginBottom: '0.65rem' }}>現状の課題</div>
                <div>{selected.issues.map(v => <Tag key={v} label={v} color="#1a2540" bg="#f0f4ff" />)}</div>
              </div>
            )}
            ...
            {selected.services?.length > 0 && (
              <div style={{ background: '#fff', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1rem', border: '1px solid #e2e8f0' }}>
                <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', color: '#94a3b8', marginBottom: '0.65rem' }}>興味のあるサービス</div>
                <div>{selected.services.map(v => <Tag key={v} label={v} color="#3b82f6" bg="#eff6ff" />)}</div>
              </div>
            )}
            ...
            {selected.goal && (
              <div style={{ background: '#fff', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1rem', border: '1px solid #e2e8f0' }}>
                <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', color: '#94a3b8', marginBottom: '0.65rem' }}>解決したい課題</div>
            ...
            {selected.note && (
              <div style={{ background: '#fff', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1rem', border: '1px solid #e2e8f0' }}>
                <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', color: '#94a3b8', marginBottom: '0.65rem' }}>その他メモ（顧客記入）</div>
            ...
            <div style={{ background: '#fff', borderRadius: 10, padding: '1rem 1.25rem', border: '1px solid #e2e8f0' }}>
              <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', color: '#94a3b8', marginBottom: '0.65rem' }}>
                商談メモ <span style={{ fontWeight: 400 }}>（自動保存）</span>
              </div>
              <textarea
                value={memo}
